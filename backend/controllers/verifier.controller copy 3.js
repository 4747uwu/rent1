...
const buildVerifierBaseQuery = (req, workflowStatuses = null) => {
    ...
    const queryFilters = {
        organizationIdentifier: user.organizationIdentifier,  // ✅ FIXED: direct match, not $or
        workflowStatus: { 
            $in: [
    ...
    if (assignedRadiologists.length > 0) {
        // Only show studies assigned to the verifier's linked radiologists
        ...
        // No radiologists linked = see all studies in org (admin-style verifier)
        console.log('🔓 [Verifier] No radiologist restriction - seeing all org studies');
    }

    // ✅ CRITICAL: COMBINE FILTERS WITH $OR LOGIC
    // Show study if:
    // - Both radiologist AND lab filters exist → study matches EITHER one
    // - Only radiologist filter exists → use radiologist filter only
    // - Only lab filter exists → use lab filter only
    // - Neither filter exists → show all (no restriction)
    
    if (radiologistFilter && labFilter) {
        // Both filters exist: study should match EITHER radiologist OR lab binding
        queryFilters.$or = [
            radiologistFilter,  // Check radiologist binding
            labFilter           // OR check lab binding
        ];
    ...
    if (req.query.labId && req.query.labId !== 'all' && mongoose.Types.ObjectId.isValid(req.query.labId)) {
        // Only allow if verifier has access to this lab
    ...
    if (req.query.radiologist && req.query.radiologist !== 'all' && mongoose.Types.ObjectId.isValid(req.query.radiologist)) {
        ...
        // Only allow if this radiologist is in their assigned list (or no restriction)
