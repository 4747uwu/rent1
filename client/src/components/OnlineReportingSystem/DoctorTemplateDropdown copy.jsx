...
import { ChevronDown, Search, User, FileText, X, Check } from 'lucide-react';
...
const DoctorTemplateDropdown = ({ onTemplateSelect, selectedTemplate }) => {
  ...
  const fetchTemplates = async () => {
    setLoading(true);
    try {
      const res = await api.get('/html-templates/doctor/my-templates', { params: { category: selectedCategory, search: searchTerm.trim(), limit: 100 } });
  ...
  const isActive = selectedTemplate?.templateScope === 'doctor_specific';
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button onClick={() => setIsOpen(!isOpen)}
        ...
        <User size={10} />
        <span>Mine</span>
        {total > 0 && <span className="bg-blue-100 text-blue-700 text-[9px] px-1 rounded-full font-bold">{total}</span>}
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-0.5 w-60 bg-white rounded-md shadow-xl border border-gray-200 z-50">
          ...
          <div className="p-1.5 border-b border-gray-100 space-y-1">
            <div className="relative">
              ...
              <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search..."
                className="w-full pl-5 pr-5 py-0.5 text-[10px] border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-400" />
            ...
            <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)}
              className="w-full text-[10px] border border-gray-200 rounded px-1 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-400">
          ...
          <div className="max-h-64 overflow-y-auto">
            {loading ? (
              <div className="p-2 text-center"><div className="animate-spin h-3 w-3 border-2 border-blue-500 border-t-transparent rounded-full mx-auto" /></div>
            ) : total === 0 ? (
              <div className="p-3 text-center"><FileText size={14} className="text-gray-300 mx-auto mb-1" /><p className="text-[10px] text-gray-400">{searchTerm ? 'No results' : 'No templates'}</p></div>
            ) : (
                ...
                <div key={cat}>
                  ...
                  {list.map(t => (
                    <button key={t._id} onClick={() => { onTemplateSelect(t); setIsOpen(false); }}
                      className={`w-full px-2 py-1 text-left text-[10px] hover:bg-blue-50 flex items-center gap-1.5 ${selectedTemplate?._id === t._id ? 'bg-blue-50 border-r-2 border-blue-500' : ''}`}>
                      <span className="flex-1 truncate font-medium text-gray-800">{t.title}</span>
                      {selectedTemplate?._id === t._id && <Check size={10} className="text-blue-600 shrink-0" />}
                    </button>
                  ))}
                </div>
              ))
