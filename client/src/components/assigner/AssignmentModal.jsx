...
const AssignmentModalContent = ({ 
  ...
  return (
    ...
    <div 
      ...
      style={{
        top: `${position?.top || 0}px`, left: `${position?.left || 0}px`, width: `${position?.width || 320}px`,
        maxWidth: '350px', maxHeight: '280px'
      }}
    >
      {/* ✅ HEADER: Matched theme */}
      <div className="flex items-center justify-between px-2 py-1 bg-gray-900 text-white rounded-t">
        <div className="flex items-center gap-1 min-w-0">
          <User className="w-3 h-3 flex-shrink-0" />
          <div className="truncate">
            <div className="text-[9px] font-bold uppercase">Assign Radiologists</div>
            {searchTerm && <div className="text-[7px] text-gray-300 uppercase truncate">Search: "{searchTerm}"</div>}
          </div>
        </div>
        <button onClick={onClose} className="p-0.5 hover:bg-gray-700 rounded transition-colors" disabled={loading}>
          <X className="w-3 h-3" />
        </button>
      </div>
      ...
      <div className="px-2 py-1 bg-gray-50 border-b border-gray-200">
        <div className="text-[8px] text-gray-800 uppercase font-semibold leading-tight">
            ...
            {filteredRadiologists.map((radiologist) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center px-2 py-1 hover:bg-gray-50 cursor-pointer transition-colors ${
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[7px] text-gray-500 font-medium uppercase flex items-center gap-1.5">
                      {stats && (
                        <>
                          <span className="text-blue-600">{stats.assignedTodayCount}A</span>
                          <span className="text-amber-600">{stats.pendingTodayCount}P</span>
                        </>
                      )}
                      {hasWorkload && (
                        <span>{radiologist.workload.currentWorkload} Active</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {/* ✅ FOOTER */}
      <div className="px-2 py-1.5 bg-gray-50 border-t border-gray-200 rounded-b flex gap-1.5">
        <button onClick={onClose} disabled={loading} className="px-2 py-0.5 text-[8px] font-bold bg-gray-200 text-gray-700 rounded border border-gray-300 uppercase hover:bg-gray-300 w-1/3">
          Cancel
        </button>
        <button onClick={handleApplyAssignments} disabled={loading} className="px-2 py-0.5 text-[8px] font-bold bg-gray-900 text-white rounded border border-gray-900 uppercase hover:bg-black w-2/3">
